...
for cn in $(sdc-listmachines --url $ams1 2>/dev/null |json -ag ips[0])
 do
        ...
        ssh -o "StrictHostKeyChecking no" root@$cn wget --no-check-certificate http://smartos.org/ > /dev/null 2>&1 &
        ssh -o "StrictHostKeyChecking no" root@$cn wget --no-check-certificate http://elementary.io/ > /dev/null 2>&1 &
 done
